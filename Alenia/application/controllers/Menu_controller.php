...
class Menu_controller extends CI_Controller
{
	...
	public function index(){
		...
		$data ['menu']= $this->Menu_Pakaian->tampilMenu();
	...
	public function tambahMenu(){
		$cekKode = $this->Menu_Pakaian->cekKodeMenu();
		if ($cekKode=="NULL") {
			$cekKode="0000";
		}
		$noUrut = (substr($cekKode, 0, -3)) + 1;
		if ($this->input->post('type')=="pakaian") {
			$kd_menu = $noUrut.'PKN';
		}
		elseif ($this->input->post('type')=="sepatu") {
			$kd_menu = $noUrut.'SPT';
		}
		elseif ($this->input->post('type')=="tas") {
			$kd_menu = $noUrut.'TAS';
		}
		...
		if ( ! $this->upload->do_upload('upload')){
			...
			$this->Menu_Pakaian->tambahMenu($kd_menu, 'default.jpg');
			redirect('Index_controller','refresh');	
		}else{
			$this->session->set_flashdata('successUploadAdd', 'Menu berhasil ditambahkan.');
			$this->Menu_Pakaian->tambahMenu($kd_menu, $this->upload->data('file_name'));
			redirect('Index_controller','refresh');	
		}
	}
	...
	public function editMenu(){
		if ($this->input->post('kd_menu') == NULL) {
			$this->Menu_Pakaian->editMenu($kd_menu, $cekImg);
			redirect('Index_controller','refresh');
		}
		else {
			$kd_menu = $this->input->post('kd_menu');
			$cekImg = $this->Menu_Pakaian->cekImg($this->input->post('kd_menu'));
			...
			if ( ! $this->upload->do_upload('upload')){
				...
				$this->Menu_Pakaian->editMenu($kd_menu, $this->upload->data('file_name'));
				if ($cekImg!='default.jpg') {
	...
	{
		$cekImg = $this->Menu_Pakaian->cekImg($kd_menu);
		if ($cekImg!='default.jpg') {
			unlink('./upload/menu/'.$cekImg);
		}
		$this->Menu_Pakaian->hapusMenu($kd_menu);
