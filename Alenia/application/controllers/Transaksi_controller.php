...
class Transaksi_controller extends CI_Controller
{
	...
	public function inputPembayaran(){
		...
		foreach ($content as $item) {
			$cekStok=$this->Menu_Pakaian->cekStok($item['id']);
			if ($item['qty']>$cekStok) {
		...
		if ($this->input->post('type')=='tunai') {
			if ($cek == "True") {
				...
				if ($uang < $total) {
					...
					fputs($location, "           Aline" . "\n");
					fputs($location, "     Jl.A.Yani No.6," . "\n");
					fputs($location, "    Pangongangan, Kec.Manguharjo" . "\n");
					fputs($location, "        Kota Madiun, 63121" . "\n");
					fputs($location, "                             " . "\n");
